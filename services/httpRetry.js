...
async function fetchWithRetry(url, init, opts = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(`[httpRetry] ${url.split('?')[0]} → ${resp.status}, retrying in ${delay}ms (attempt ${attempt}/${maxAttempts})`);
      await sleep(delay);
    } catch (err) {
      // Network / DNS / abort error: retry.
      lastErr = err;
      if (attempt === maxAttempts) throw err;
      const delay = computeBackoff(attempt, null, baseMs, maxMs);
      console.warn(`[httpRetry] network error: ${err.message}, retrying in ${delay}ms (attempt ${attempt}/${maxAttempts})`);
      await sleep(delay);
    }
  }
